...
object Libs {
	...
	fun copyJniHeaders(program: AstProgram) {
		...
		File(jniDir, "jni.h").writeBytes(program.resourcesVfs["cpp/jni-headers/jni.h"].readBytes())
		File(jniDir, "jawt.h").writeBytes(program.resourcesVfs["cpp/jni-headers/jawt.h"].readBytes())
		File(jniDir, "classfile_constants.h").writeBytes(program.resourcesVfs["cpp/jni-headers/classfile_constants.h"].readBytes())
		File(jniDir, "jdwpTransport.h").writeBytes(program.resourcesVfs["cpp/jni-headers/jdwpTransport.h"].readBytes())
		os.forEach {
			val osDir = File(jniDir, it)
			osDir.ensureExists()
			File(osDir, "jni_md.h").writeBytes(program.resourcesVfs["cpp/jni-headers/${it}/jni_md.h"].readBytes())
			if (it != "mac") File(osDir, "jawt_md.h").writeBytes(program.resourcesVfs["cpp/jni-headers/${it}/jawt_md.h"].readBytes())
		}
	}
}