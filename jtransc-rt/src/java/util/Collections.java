...
public class Collections {
    ...
    static class SynchronizedMap<K, V> implements Map<K, V>, Serializable {
        ...
        @Override public Set<Entry<K, V>> entrySet() {
            synchronized (mutex) {
                return new SynchronizedSet<Entry<K, V>>(m.entrySet(), mutex);
            }
        }
    ...
    private static class UnmodifiableMap<K, V> implements Map<K, V>, Serializable {
        ...
        private static class UnmodifiableEntrySet<K, V> extends UnmodifiableSet<Entry<K, V>> {
            ...
            @Override public Iterator<Entry<K, V>> iterator() {
                return new Iterator<Entry<K, V>>() {
                    Iterator<Entry<K, V>> iterator = c.iterator();
    ...
    private static class CheckedMap<K, V> implements Map<K, V>, Serializable {
        ...
        @Override public void putAll(Map<? extends K, ? extends V> map) {
            ...
            Iterator<? extends Entry<? extends K, ? extends V>> it = map
                    .entrySet().iterator();
            for (int i = 0; i < size; i++) {
        ...
        private static class CheckedEntry<K, V> implements Entry<K, V> {
            ...
            public CheckedEntry(Entry<K, V> e, Class<V> valueType) {
                if (e == null) {
        ...
        private static class CheckedEntrySet<K, V> implements Set<Entry<K, V>> {
            ...
            public CheckedEntrySet(Set<Entry<K, V>> s, Class<V> valueType) {
                this.s = s;
                this.valueType = valueType;
            }
            ...
            @Override public boolean add(Entry<K, V> o) {
                throw new UnsupportedOperationException();
            }
            ...
            private static class CheckedEntryIterator<K, V> implements Iterator<Entry<K, V>> {
                ...
                public CheckedEntryIterator(Iterator<Entry<K, V>> i,
                        Class<V> valueType) {
                    this.i = i;
                    this.valueType = valueType;
                }
