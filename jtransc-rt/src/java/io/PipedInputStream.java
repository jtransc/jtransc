...
public class PipedInputStream extends InputStream {
     ...
     * @throws IOException if this stream or {@code out} are already connected.
     */
     ...
     * reached.
     * @throws IOException if this stream is closed or not connected to an output
     ...
     * has been reached.
     *
     * @throws IndexOutOfBoundsException if {@code byteOffset < 0 || byteCount < 0 || byteOffset + byteCount > bytes.length}.
     * @throws InterruptedIOException    if the thread reading from this stream is interrupted.
     * @throws IOException               if this stream is closed or not connected to an output
     *                                   stream, or if the thread writing to the connected output
     *                                   stream is no longer alive.
    ...
    public synchronized int read(byte[] bytes, int byteOffset, int byteCount) throws IOException {
        JTranscArrays.checkOffsetAndCount(bytes.length, byteOffset, byteCount);
        if (byteCount == 0) {
            return 0;
        }
     ...
     * @throws InterruptedIOException if the {@code buffer} is full and the thread that has called
     *                                this method is interrupted.
     * @throws IOException            if this stream is closed or the thread that has last read
