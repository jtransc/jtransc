	...
	while (i != null) {
		when (i) {
			...
			is TypeInsnNode -> {
				...
				out.add(when (i.opcode) {
					Opcodes.NEW -> BAF.ANEW(type)
					Opcodes.ANEWARRAY -> BAF.NEWARRAY(type, 1)
					Opcodes.CHECKCAST -> BAF.CHECKCAST(type)
					Opcodes.INSTANCEOF -> BAF.AINSTANCEOF(type)
			...
			is MethodInsnNode -> {
				out.add(BAF.INVOKE(i.owner, i.name, i.desc, i.itf, when (i.opcode) {
			...
			is FrameNode -> {
				...
				throw InvalidOperationException("$i")
			}
		}
		i = i.next
	}
