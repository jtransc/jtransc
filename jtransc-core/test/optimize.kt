...
class OptimizeTest {
	...
	@Test fun test1() {
		val expr = build { null.lit.cast(OBJECT).cast(CLASS).cast(STRING) }
	...
	@Test fun test2() {
		Assert.assertEquals("1", build { 1.lit.cast(INT) }.optimize(flags).exprDump())
	...
	@Test fun test4() {
		...
		Assert.assertEquals("false", build { 0.lit.cast(BOOL) }.optimize(flags).exprDump())
		Assert.assertEquals("true", build { 1.lit.cast(BOOL) }.optimize(flags).exprDump())
	}
